...
public class NavHistoryActivity extends AppCompatActivity {
    ...
    public void getSwagTubeData() {
        if (App.isOnline()) {
            ...
            Call<SwagTubeResponse> userResponseCall = apiInterface.getWatchHistory(PrefConnect.readString(NavHistoryActivity.this, Constants.USERID, ""));
                    ...
                    if (response.code() == Constants.SUCCESS) {
                        if (response.body().getStatus().equals("1")) {
                            ...
                            if (swagTubeDataList.size() != 0 && swagTubeDataList != null && !swagTubeDataList.isEmpty()) {
                                NavWatchAdapter swagTubeAdapter = new NavWatchAdapter(NavHistoryActivity.this, swagTubeDataList, "history");
                        ...
                        try {
                            ...
                            toast(NavHistoryActivity.this, jObjError.getString("response_msg"));
                        ...
                        OwnerGlobal.LoginRedirect(NavHistoryActivity.this);
                    }
                }
    ...
    private void removeVideoFromList() {
        ProgressDialog progressDialog = new ProgressDialog(NavHistoryActivity.this);
        progressDialog.setMessage("Please wait...");
        progressDialog.show();
        Api apiInterface = RetrofitClient.getInstance().getApi();
        Call<RemoveDataResponse> removeDataResponseCall = apiInterface.deleteReportVideo("", PrefConnect.readString(NavHistoryActivity.this, Constants.USERID, ""));
        removeDataResponseCall.enqueue(new Callback<RemoveDataResponse>() {
            ...
            public void onResponse(Call<RemoveDataResponse> call, Response<RemoveDataResponse> response) {
                ...
                if (response.code() == 200) {
                    if (response.body().getStatus().equals("1")) {
                        OwnerGlobal.toast(NavHistoryActivity.this, response.body().getMessage());
                        nothing_main_layout.setVisibility(View.VISIBLE);
                        historyLayout.setVisibility(View.GONE);
                    } else {
                        OwnerGlobal.toast(NavHistoryActivity.this, response.body().getMessage());
